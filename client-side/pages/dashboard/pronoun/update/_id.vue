...
export default {
    ...
    created() {
        this.$store.commit('setActiveSidebar', { isActive: 'translate', subIsActive: 'pronoun' });
        this.initPronoun();
    },
    methods: {
        initPronoun: function() {
            this.isLoad = true;
            this.$http.get('translate/update/' + this.$route.params.id + '?type=pronoun')
            .then( response => {
